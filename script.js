...
function loginUser() {
  ...
  // Password expired → allow username-only login
  if (remaining <= 0) {
    localStorage.removeItem(userKey); // remove password protection
    localStorage.setItem('current_user', username);
    document.getElementById('message').innerText = '✅ Password expired. Logging in with username only.';
    setTimeout(() => window.location.href = 'notes.html', 1000);
    return;
  }

  // Password field is blank → show countdown time left
  if (!password) {
...
function showCountdown() {
  ...
  if (!userDataRaw) return; // expired already
